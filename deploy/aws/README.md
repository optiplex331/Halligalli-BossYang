...
HCP Terraform stores remote state and state versions. It does not execute Terraform runs for this scaffold phase.
...
- `apply` creates or changes infrastructure after protected approval and `PRODUCTION_SCAFFOLD_APPLY`.
- `scale-down` applies the same root with `backend_desired_count=0` after `PRODUCTION_SCAFFOLD_SCALE_DOWN`.
- `destroy` runs Terraform destroy after protected approval and `PRODUCTION_SCAFFOLD_DESTROY`.

Do not run `terraform apply` as a validation shortcut. A real apply creates cost-bearing AWS resources and should happen only through the protected infrastructure workflow after reviewing the plan, HCP state workspace, AWS role, DNS ownership, certificate setup, and scaffold lifecycle.
