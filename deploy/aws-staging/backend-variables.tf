...
variable "backend_image_tag" {
  description = "AWS Staging Image tag expected in the ECR backend repository."
  type        = string
...
variable "backend_task_cpu" {
  description = "Fargate CPU units for the single AWS Staging backend task."
  type        = number
  default     = 256

  validation {
    condition     = contains([256, 512, 1024], var.backend_task_cpu)
    error_message = "Backend task CPU must use a small Fargate size suitable for staging demos."
  }
}

variable "backend_task_memory" {
  description = "Fargate memory MiB for the single AWS Staging backend task."
  type        = number
  default     = 512

  validation {
    condition     = contains([512, 1024, 2048], var.backend_task_memory)
    error_message = "Backend task memory must use a small Fargate size suitable for staging demos."
  }
}

variable "backend_app_version" {
  description = "Release Identity placeholder for APP_VERSION until the manual AWS Staging deployment workflow injects a release value."
  type        = string
  default     = "aws-staging-placeholder"
...
variable "backend_commit_sha" {
  description = "Release Identity placeholder for COMMIT_SHA until the manual AWS Staging deployment workflow injects a commit SHA."
  type        = string
