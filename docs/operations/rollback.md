...
Rollback for AWS Production is an application deployment operation. Terraform should not be used for ordinary application rollback unless the infrastructure itself is broken.

## Preferred Rollback

1. Find a known-good GitHub Release, commit SHA, or previously deployed ECR image tag from workflow logs.
2. Run `AWS Production` with `operation=deploy-backend`, `backend_image_tag=<known-good-tag>`, and `confirm_cost=AWS_PRODUCTION_APPLY` if the backend image is already in ECR.
...
If production must recover before a clean redeploy can complete, use AWS console or CLI controls to point the ECS service at a previous healthy task definition revision, then follow with a normal `AWS Production` workflow run so GitHub Actions logs capture the restored runtime identity.
...
- Do not bypass the protected `aws-production` GitHub Environment for normal rollback.
