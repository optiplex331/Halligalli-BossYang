...
Terraform state is local for the current AWS Staging/Portfolio phase. Keep `terraform.tfstate`, `.terraform/`, plans, and local tfvars ignored.

Do not migrate AWS Staging/Portfolio to remote Terraform state merely because the environment is live. Remote state is deferred until AWS becomes the production path.
...
## Activating A Real Staging Environment

Use this order when turning the reference into a live AWS Staging/Portfolio environment with local Terraform state. Do not skip the bootstrap sequence: the first ECS service cannot run a real task until the ECR repository exists and contains an image.

1. Fill `deploy/aws/environments/staging/terraform.tfvars`.
...
7. Copy Terraform outputs into the GitHub `aws-staging` environment variables listed below.
...
10. Run the `AWS Staging Scaffold` workflow with `operation=deploy-backend` and `confirm_cost=STAGING_APPLY`.
11. Run `operation=deploy-frontend` with `confirm_cost=STAGING_APPLY`.
...
Terraform owns the baseline ECS task definition, but normal AWS Staging image rollout is workflow-owned. The ECS service ignores `task_definition` drift so a later local Terraform apply does not roll the service back from a workflow-deployed image revision. Do not remove that drift guard unless Terraform becomes the owner of AWS Staging application rollout.
