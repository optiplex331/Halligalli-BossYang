...
`AWS Staging Scaffold` workflow 只通过 `workflow_dispatch` 触发，不挂在 `push`、PR 或 Release Tag 上。默认 `validate` 操作只运行 release config 和 Terraform 静态校验；`deploy-frontend` 和 `deploy-backend` 必须手动输入 `STAGING_APPLY` 才会执行 AWS-mutating 步骤。

AWS Staging/Portfolio 变更属于 Delivery Control。`deploy/aws-staging/**` 和 `.github/workflows/aws-staging.yml` 会让 `Product checks` 运行 release utility validation 与 actionlint，但不会自动发布 AWS 资源，也不会改变 DO Production。

AWS Staging 的操作细节记录在 [AWS staging scaffold](aws-staging.md)。
