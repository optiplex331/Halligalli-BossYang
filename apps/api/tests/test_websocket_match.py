...
from redis_test_case import RedisTestCase, verified_credential
    ...
    def test_two_websocket_players_ready_start_and_receive_a_bell_result(self) -> None:
        ...
        with TestClient(create_app(authority=authority)) as client:
            created = client.post(
                ...
                json={"name": "Host", "credentialVerifier": verified_credential(host_credential), "tableSeatCount": 4, "targetHumanParticipantCount": 2, "difficulty": "normal", "durationSec": 60},
            )
            ...
            client.post(
                ...
                json={"name": "Guest", "credentialVerifier": verified_credential(guest_credential)},
            )
